...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error("[generate-image] Failed to parse JSON. Length:", rawText.length, "Tail:", rawText.slice(-100));
      return new Response(
    ...
    if (finishReason === "length") {
      console.warn("[generate-image] Response truncated (finish_reason=length)");
    }
    ...
    return new Response(
      JSON.stringify({ error: "No image generated by the model. Please try again." }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (e) {
