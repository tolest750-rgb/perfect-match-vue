import type { ProcessedSlide, LightKey } from "./parser";

// ─── AI LAYOUT INTERFACE (embutido — sem arquivo separado) ────
export interface AILayout {
  titleX: number;
  titleY: number;
  titleAlign: "left" | "center" | "right";
  titleMaxWidthRatio: number;
  subtitleX: number;
  subtitleY: number;
  subtitleAlign: "left" | "center" | "right";
  subtitleMaxWidthRatio: number;
  ctaX: number;
  ctaY: number;
  gradientType: "bottom" | "top" | "left" | "right" | "radial-center";
  gradientStart: number;
  gradientMaxOpacity: number;
}

export const DEFAULT_LAYOUT: AILayout = {
  titleX: 0.07,
  titleY: 0.7,
  titleAlign: "left",
  titleMaxWidthRatio: 0.88,
  subtitleX: 0.07,
  subtitleY: 0.82,
  subtitleAlign: "left",
  subtitleMaxWidthRatio: 0.88,
  ctaX: 0.07,
  ctaY: 0.91,
  gradientType: "bottom",
  gradientStart: 0.42,
  gradientMaxOpacity: 0.84,
};

export async function analyzeLayout(
  imageBase64: string,
  titulo: string,
  subtitulo: string,
  hasCta: boolean,
  fmt: string,
): Promise<AILayout> {
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "claude-haiku-4-5-20251001",
        max_tokens: 512,
        system: `You are a world-class editorial art director for cinematic social media.
Your job: find the CLEANEST, most visually quiet zone of the image and place text there.
Rules:
- SCAN all four quadrants (top-left, top-right, bottom-left, bottom-right)
- PRIORITIZE top placement if top zone is cleaner/darker than bottom — do not default to bottom
- Text must NEVER overlap bright faces, eyes, or busy scene detail
- If subject is at bottom, put text at top. If subject at top, put text at bottom. If centered, use a side.
- gradientType must match where text goes: top zone = "top", right = "right", bottom = "bottom", etc.
- gradientMaxOpacity: use minimum needed — prefer 0.60–0.78, only go higher if zone is very bright
- titleY must be > 0.12 (top padding) and at least 0.08 above subtitleY
- subtitleY must be at least 0.06 above ctaY
Respond ONLY with valid JSON. No explanation, no markdown, no backticks.`,
        messages: [
          {
            role: "user",
            content: [
              {
                type: "image",
                source: { type: "base64", media_type: "image/jpeg", data: imageBase64 },
              },
              {
                type: "text",
                text: `Analyze this ${fmt} image for text overlay placement.

Title: "${titulo}"
Subtitle: "${subtitulo}"
Has CTA: ${hasCta}

STEP 1: Scan all four quadrants. Identify which has the least visual information (darkest, most blurred, most empty).
STEP 2: Place the text block in that quadrant. Top zones → titleY between 0.12–0.38. Bottom zones → titleY between 0.60–0.82. Side zones → titleY between 0.25–0.65.
STEP 3: Set gradientType to match the text zone direction.
STEP 4: Set gradientMaxOpacity to the minimum needed for legibility (prefer 0.60–0.78).

Return ONLY this JSON:
{
  "titleX": <0.04–0.93>,
  "titleY": <0.12–0.82>,
  "titleAlign": <"left"|"center"|"right">,
  "titleMaxWidthRatio": <0.40–0.92>,
  "subtitleX": <0.04–0.93>,
  "subtitleY": <titleY+0.08 to titleY+0.22>,
  "subtitleAlign": <"left"|"center"|"right">,
  "subtitleMaxWidthRatio": <0.40–0.92>,
  "ctaX": <0.04–0.88>,
  "ctaY": <subtitleY+0.06 to subtitleY+0.14>,
  "gradientType": <"bottom"|"top"|"left"|"right"|"radial-center">,
  "gradientStart": <0.20–0.68>,
  "gradientMaxOpacity": <0.55–0.88>
}`,
              },
            ],
          },
        ],
      }),
    });

    if (!response.ok) return DEFAULT_LAYOUT;

    const data = await response.json();
    const raw = (data.content ?? []).map((c: any) => c.text || "").join("");
    const clean = raw.replace(/```json|```/g, "").trim();
    const p = JSON.parse(clean) as Partial<AILayout>;

    const clamp = (v: number, min: number, max: number) =>
      isFinite(v) ? Math.min(max, Math.max(min, v)) : (min + max) / 2;

    return {
      titleX: clamp(p.titleX ?? DEFAULT_LAYOUT.titleX, 0.04, 0.93),
      titleY: clamp(p.titleY ?? DEFAULT_LAYOUT.titleY, 0.12, 0.82),
      titleAlign: p.titleAlign ?? DEFAULT_LAYOUT.titleAlign,
      titleMaxWidthRatio: clamp(p.titleMaxWidthRatio ?? DEFAULT_LAYOUT.titleMaxWidthRatio, 0.4, 0.92),
      subtitleX: clamp(p.subtitleX ?? DEFAULT_LAYOUT.subtitleX, 0.04, 0.93),
      subtitleY: clamp(p.subtitleY ?? DEFAULT_LAYOUT.subtitleY, 0.2, 0.92),
      subtitleAlign: p.subtitleAlign ?? DEFAULT_LAYOUT.subtitleAlign,
      subtitleMaxWidthRatio: clamp(p.subtitleMaxWidthRatio ?? DEFAULT_LAYOUT.subtitleMaxWidthRatio, 0.4, 0.92),
      ctaX: clamp(p.ctaX ?? DEFAULT_LAYOUT.ctaX, 0.04, 0.88),
      ctaY: clamp(p.ctaY ?? DEFAULT_LAYOUT.ctaY, 0.26, 0.96),
      gradientType: p.gradientType ?? DEFAULT_LAYOUT.gradientType,
      gradientStart: clamp(p.gradientStart ?? DEFAULT_LAYOUT.gradientStart, 0.2, 0.68),
      gradientMaxOpacity: clamp(p.gradientMaxOpacity ?? DEFAULT_LAYOUT.gradientMaxOpacity, 0.55, 0.88),
    };
  } catch {
    return DEFAULT_LAYOUT;
  }
}

// ─── CANVAS HELPERS ───────────────────────────────────────────
function wrapTxt(ctx: CanvasRenderingContext2D, txt: string, font: string, maxW: number, maxL: number): string[] {
  ctx.font = font;
  const words = txt.split(" ");
  const lines: string[] = [];
  let cur = "";
  for (const w of words) {
    const t = cur ? cur + " " + w : w;
    if (ctx.measureText(t).width > maxW && cur) {
      lines.push(cur);
      if (lines.length >= maxL) {
        lines[lines.length - 1] += "…";
        return lines;
      }
      cur = w;
    } else cur = t;
  }
  if (cur) lines.push(cur);
  return lines;
}

function rrect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number) {
  ctx.beginPath();
  ctx.moveTo(x + r, y);
  ctx.lineTo(x + w - r, y);
  ctx.quadraticCurveTo(x + w, y, x + w, y + r);
  ctx.lineTo(x + w, y + h - r);
  ctx.quadraticCurveTo(x + w, y + h, x + w - r, y + h);
  ctx.lineTo(x + r, y + h);
  ctx.quadraticCurveTo(x, y + h, x, y + h - r);
  ctx.lineTo(x, y + r);
  ctx.quadraticCurveTo(x, y, x + r, y);
  ctx.closePath();
}

function hexToRgb(hex: string): string | null {
  const r = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
  return r ? `${parseInt(r[1], 16)},${parseInt(r[2], 16)},${parseInt(r[3], 16)}` : null;
}

function upscale2x(src: HTMLCanvasElement): HTMLCanvasElement {
  const up = document.createElement("canvas");
  up.width = src.width * 2;
  up.height = src.height * 2;
  const uc = up.getContext("2d")!;
  uc.imageSmoothingEnabled = true;
  uc.imageSmoothingQuality = "high";
  uc.drawImage(src, 0, 0, up.width, up.height);
  return up;
}

// ─── LOGO BRITTO* ─────────────────────────────────────────────
// Logo embutida em base64 — sem import, sem fetch, sem falha
const BRITTO_LOGO_B64 =
  "";

// ─── CONFIGURAÇÕES ────────────────────────────────────────────
const DIM: Record<string, [number, number]> = {
  "4:5": [1080, 1350],
  "9:16": [1080, 1920],
  "1:1": [1080, 1080],
};

const ACC: Record<LightKey, string> = {
  dramatic: "#00b4ff",
  warm: "#f5c842",
  green: "#c8ff00",
  moody: "#e0e0ff",
};

// ─── FONT LOADER ─────────────────────────────────────────────
let fontLoaded = false;
async function ensureFont() {
  if (fontLoaded) return;
  try {
    const font = new FontFace(
      "Bricolage Grotesque",
      "url(https://fonts.gstatic.com/s/bricolagegrotesque/v8/3y9U6as8bTXq_nANBjzKo3IeZx8z6up3BfSQCpTXABw.woff2)",
      { weight: "100 900", style: "normal" },
    );
    const loaded = await font.load();
    document.fonts.add(loaded);
    fontLoaded = true;
  } catch {
    /* fallback silencioso */
  }
}

// ─── LOGO DRAW ───────────────────────────────────────────────
function drawLogo(ctx: CanvasRenderingContext2D, CW: number, CH: number, F: number): Promise<void> {
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = () => {
      // Largura da logo = 28% da largura do canvas
      const logoW = CW * 0.28;
      const logoH = logoW * (img.height / img.width);
      // Posição: canto inferior direito, com padding
      const PAD = CW * 0.055;
      const logoX = CW - logoW - PAD;
      const logoY = CH - logoH - PAD * 0.9;

      // Halo sutil atrás da logo
      ctx.save();
      ctx.shadowColor = "rgba(0,0,0,0.55)";
      ctx.shadowBlur = 28 * F;
      ctx.globalAlpha = 0.92;
      ctx.drawImage(img, logoX, logoY, logoW, logoH);
      ctx.restore();
      resolve();
    };
    img.onerror = () => resolve(); // falha silenciosa
    img.src = "data:image/jpeg;base64," + BRITTO_LOGO_B64;
  });
}

// ─── COMPOSE SLIDE ────────────────────────────────────────────
export async function composeSlide(
  imgSrc: string | null,
  sl: ProcessedSlide,
  faceB64: string,
  aiLayout?: AILayout,
  isFirstOrLast?: boolean,
): Promise<Blob> {
  await ensureFont();

  const L = aiLayout ?? DEFAULT_LAYOUT;
  const [W, H] = DIM[sl.fmt] || [1080, 1350];
  const SC = sl.res === "4K" ? 1 : 2;
  const CW = W * SC,
    CH = H * SC,
    F = SC;

  const canvas = document.createElement("canvas");
  canvas.width = CW;
  canvas.height = CH;
  const ctx = canvas.getContext("2d")!;
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = "high";

  const PAD_X = Math.round(CW * 0.07);
  const PAD_Y = Math.round(CH * 0.055);

  const accent: string = (sl.layout as any)?.accent ?? ACC[sl.light as LightKey] ?? "#c8ff00";
  const accentRgb = hexToRgb(accent) ?? "200,255,0";

  // ── Tipografia ──────────────────────────────────────────────
  const NUM_SIZE = Math.round(14 * F);
  const TTL_SIZE = Math.round(96 * F);
  const SUB_SIZE = Math.round(34 * F);
  const CTA_SIZE = Math.round(20 * F);

  const numFont = `700 ${NUM_SIZE}px 'Bricolage Grotesque', sans-serif`;
  const tFont = `800 ${TTL_SIZE}px 'Bricolage Grotesque', sans-serif`;
  const sFont = `400 ${SUB_SIZE}px 'Bricolage Grotesque', sans-serif`;
  const ctaFont = `700 ${CTA_SIZE}px 'Bricolage Grotesque', sans-serif`;

  // ── Helpers ─────────────────────────────────────────────────
  const px = (r: number) => r * CW;
  const py = (r: number) => r * CH;

  return new Promise<Blob>((resolve) => {
    const exportBlob = async () => {
      // Adiciona logo no 1º e último slide
      if (isFirstOrLast) await drawLogo(ctx, CW, CH, F);
      const out = upscale2x(canvas);
      out.toBlob((b) => resolve(b!), "image/png", 1.0);
    };

    const doText = () => {
      // ── 1. GRADIENTE LEGIBILIDADE ──────────────────────────
      let ov: CanvasGradient;
      const mo = L.gradientMaxOpacity;
      const gs = L.gradientStart;
      switch (L.gradientType) {
        case "top":
          ov = ctx.createLinearGradient(0, 0, 0, CH * (1 - gs));
          ov.addColorStop(0, `rgba(0,0,0,${mo})`);
          ov.addColorStop(0.5, `rgba(0,0,0,${mo * 0.55})`);
          ov.addColorStop(1, "rgba(0,0,0,0)");
          break;
        case "left":
          ov = ctx.createLinearGradient(0, 0, CW * (1 - gs), 0);
          ov.addColorStop(0, `rgba(0,0,0,${mo})`);
          ov.addColorStop(0.5, `rgba(0,0,0,${mo * 0.5})`);
          ov.addColorStop(1, "rgba(0,0,0,0)");
          break;
        case "right":
          ov = ctx.createLinearGradient(CW, 0, CW * gs, 0);
          ov.addColorStop(0, `rgba(0,0,0,${mo})`);
          ov.addColorStop(0.5, `rgba(0,0,0,${mo * 0.5})`);
          ov.addColorStop(1, "rgba(0,0,0,0)");
          break;
        case "radial-center":
          ov = ctx.createRadialGradient(CW / 2, CH / 2, CH * 0.08, CW / 2, CH / 2, CH * 0.72);
          ov.addColorStop(0, "rgba(0,0,0,0)");
          ov.addColorStop(1, `rgba(0,0,0,${mo})`);
          break;
        default: // bottom
          ov = ctx.createLinearGradient(0, CH * gs, 0, CH);
          ov.addColorStop(0, "rgba(0,0,0,0)");
          ov.addColorStop(0.28, `rgba(0,0,0,${mo * 0.38})`);
          ov.addColorStop(0.6, `rgba(0,0,0,${mo * 0.78})`);
          ov.addColorStop(1, `rgba(0,0,0,${mo})`);
      }
      ctx.fillStyle = ov;
      ctx.fillRect(0, 0, CW, CH);

      // ── 2. COR AMBIENTE ────────────────────────────────────
      let ambOv: CanvasGradient;
      switch (L.gradientType) {
        case "top":
          ambOv = ctx.createLinearGradient(0, 0, 0, CH * 0.45);
          break;
        case "left":
          ambOv = ctx.createLinearGradient(0, 0, CW * 0.45, 0);
          break;
        case "right":
          ambOv = ctx.createLinearGradient(CW, 0, CW * 0.55, 0);
          break;
        default:
          ambOv = ctx.createLinearGradient(0, CH * (gs + 0.08), 0, CH);
      }
      ambOv.addColorStop(0, `rgba(${accentRgb},0)`);
      ambOv.addColorStop(0.6, `rgba(${accentRgb},0.05)`);
      ambOv.addColorStop(1, `rgba(${accentRgb},0.12)`);
      ctx.fillStyle = ambOv;
      ctx.fillRect(0, 0, CW, CH);

      // ── 3. NÚMERO DO SLIDE ────────────────────────────────
      ctx.save();
      ctx.font = numFont;
      ctx.shadowColor = "rgba(0,0,0,0.6)";
      ctx.shadowBlur = 8 * F;
      ctx.fillStyle = "rgba(255,255,255,0.45)";
      ctx.textBaseline = "top";
      ctx.fillText(sl.num, PAD_X, PAD_Y);
      ctx.textBaseline = "alphabetic";
      ctx.restore();

      // ── 4. TÍTULO ─────────────────────────────────────────
      const titleMaxW = px(L.titleMaxWidthRatio);
      const tLines = wrapTxt(ctx, sl.titulo, tFont, titleMaxW, 3);
      const tLH = TTL_SIZE * F * 0.96;
      let curY = py(L.titleY);

      ctx.font = tFont;
      tLines.forEach((ln, idx) => {
        const isAccentLine = idx === tLines.length - 1;
        const lineX = px(L.titleX);
        if (isAccentLine) {
          ctx.save();
          ctx.textAlign = L.titleAlign;
          ctx.shadowColor = accent;
          ctx.shadowBlur = 55 * F;
          ctx.shadowOffsetY = 6 * F;
          ctx.globalAlpha = 0.6;
          ctx.fillStyle = accent;
          ctx.fillText(ln, lineX, curY);
          ctx.restore();
          ctx.save();
          ctx.textAlign = L.titleAlign;
          ctx.shadowColor = `rgba(${accentRgb},0.4)`;
          ctx.shadowBlur = 20 * F;
          ctx.shadowOffsetY = 3 * F;
          ctx.fillStyle = accent;
          ctx.fillText(ln, lineX, curY);
          ctx.restore();
        } else {
          ctx.save();
          ctx.textAlign = L.titleAlign;
          ctx.shadowColor = "rgba(0,0,0,0.80)";
          ctx.shadowBlur = 22 * F;
          ctx.shadowOffsetY = 4 * F;
          ctx.fillStyle = "#ffffff";
          ctx.fillText(ln, lineX, curY);
          ctx.restore();
        }
        curY += tLH;
      });
      ctx.textAlign = "left";

      // ── 5. SUBTÍTULO ──────────────────────────────────────
      if (sl.subtitulo) {
        const subMaxW = px(L.subtitleMaxWidthRatio);
        const sLines = wrapTxt(ctx, sl.subtitulo, sFont, subMaxW, 3);
        const sLH = SUB_SIZE * F * 1.5;
        curY = py(L.subtitleY);
        ctx.font = sFont;
        sLines.forEach((ln) => {
          ctx.save();
          ctx.textAlign = L.subtitleAlign;
          ctx.shadowColor = "rgba(0,0,0,0.70)";
          ctx.shadowBlur = 16 * F;
          ctx.shadowOffsetY = 3 * F;
          ctx.fillStyle = "rgba(255,255,255,0.88)";
          ctx.fillText(ln, px(L.subtitleX), curY);
          ctx.restore();
          curY += sLH;
        });
        ctx.textAlign = "left";
      }

      // ── 6. CTA ────────────────────────────────────────────
      if (sl.cta) {
        ctx.font = ctaFont;
        const ICON_W = 32 * F;
        const ICON_GAP = 12 * F;
        const PAD_L = 28 * F;
        const PAD_R = 20 * F;
        const tm = ctx.measureText(sl.cta);
        const ctaW = PAD_L + tm.width + ICON_GAP + ICON_W + PAD_R;
        const ctaH = 58 * F;
        const cx = px(L.ctaX);
        const cy = py(L.ctaY) - ctaH * 0.5;

        // Sombra colorida
        ctx.save();
        ctx.shadowColor = `rgba(${accentRgb},0.55)`;
        ctx.shadowBlur = 36 * F;
        ctx.shadowOffsetY = 8 * F;
        ctx.fillStyle = accent;
        rrect(ctx, cx, cy, ctaW, ctaH, 30 * F);
        ctx.fill();
        ctx.restore();

        // Fill
        ctx.fillStyle = accent;
        rrect(ctx, cx, cy, ctaW, ctaH, 30 * F);
        ctx.fill();

        // Highlight
        const hl = ctx.createLinearGradient(cx, cy, cx, cy + ctaH * 0.55);
        hl.addColorStop(0, "rgba(255,255,255,0.28)");
        hl.addColorStop(0.5, "rgba(255,255,255,0.06)");
        hl.addColorStop(1, "rgba(255,255,255,0)");
        ctx.fillStyle = hl;
        rrect(ctx, cx, cy, ctaW, ctaH * 0.55, 30 * F);
        ctx.fill();

        // Borda
        ctx.save();
        ctx.strokeStyle = "rgba(255,255,255,0.18)";
        ctx.lineWidth = 1.5 * F;
        rrect(ctx, cx, cy, ctaW, ctaH, 30 * F);
        ctx.stroke();
        ctx.restore();

        // Texto
        ctx.save();
        ctx.font = ctaFont;
        ctx.shadowColor = "rgba(0,0,0,0.30)";
        ctx.shadowBlur = 5 * F;
        ctx.shadowOffsetY = 1 * F;
        ctx.fillStyle = "#000000";
        ctx.textBaseline = "middle";
        ctx.fillText(sl.cta, cx + PAD_L, cy + ctaH * 0.5);
        ctx.textBaseline = "alphabetic";
        ctx.restore();

        // Ícone seta
        const iconCX = cx + PAD_L + tm.width + ICON_GAP + ICON_W * 0.5;
        const iconCY = cy + ctaH * 0.5;
        const iconR = ICON_W * 0.42;
        ctx.save();
        ctx.fillStyle = "rgba(0,0,0,0.22)";
        ctx.beginPath();
        ctx.arc(iconCX, iconCY, iconR, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
        ctx.save();
        ctx.strokeStyle = "#000000";
        ctx.lineWidth = 2.2 * F;
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        const aW = iconR * 0.55,
          aH = iconR * 0.45;
        ctx.beginPath();
        ctx.moveTo(iconCX - aW, iconCY);
        ctx.lineTo(iconCX + aW, iconCY);
        ctx.moveTo(iconCX + aW - aH * 0.8, iconCY - aH);
        ctx.lineTo(iconCX + aW, iconCY);
        ctx.lineTo(iconCX + aW - aH * 0.8, iconCY + aH);
        ctx.stroke();
        ctx.restore();
      }

      // Exporta (async por causa da logo)
      exportBlob();
    };

    // ── Fallback sem imagem ────────────────────────────────
    const drawFallback = () => {
      const GC: Record<string, [string, string]> = {
        dramatic: ["#030318", "#06103a"],
        warm: ["#100500", "#221000"],
        green: ["#010901", "#020f03"],
        moody: ["#080810", "#10101e"],
      };
      const colors = GC[sl.light] || ["#030318", "#06103a"];
      const grd = ctx.createLinearGradient(0, 0, 0, CH);
      grd.addColorStop(0, colors[0]);
      grd.addColorStop(1, colors[1]);
      ctx.fillStyle = grd;
      ctx.fillRect(0, 0, CW, CH);

      if (faceB64) {
        const fi = new Image();
        fi.onload = () => {
          const fh = CH * 0.55,
            fw = fh * (fi.width / fi.height);
          ctx.save();
          ctx.globalAlpha = 0.35;
          ctx.drawImage(fi, (CW - fw) / 2, CH * 0.05, fw, fh);
          ctx.restore();
          doText();
        };
        fi.onerror = () => doText();
        fi.src = "data:image/jpeg;base64," + faceB64;
      } else {
        doText();
      }
    };

    // ── Fluxo principal ───────────────────────────────────
    if (imgSrc) {
      const img = new Image();
      img.crossOrigin = "anonymous";
      img.onload = () => {
        ctx.drawImage(img, 0, 0, CW, CH);
        doText();
      };
      img.onerror = () => drawFallback();
      img.src = imgSrc;
    } else {
      drawFallback();
    }
  });
}
