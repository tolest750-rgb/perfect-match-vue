...
export function CarouselProvider({ children }: { children: React.ReactNode }) {
  ...
  const startGeneration = useCallback(async () => {
    ...
    for (let i = 0; i < processedSlides.length; i++) {
      ...
      try {
        const imgJobs = [0, 1, 2, 3].map(v =>
          callGemini(processedSlides[i], v, apiKey, faceB64Ref.current, demo)
        );
  ...
  const getVarBlob = useCallback((slideIdx: number, varIdx: number) => {
    return composedBlobs[slideIdx]?.[varIdx] || null;
  }, [composedBlobs]);

  const value: CarouselState & CarouselActions = {
    faceB64, faceDataUrl, faceName, apiKey, demo, style, light, fmt, res, rawText,
    slides, composedBlobs, varUrls, varStatuses, slideStatuses, slideSteps,
    isGenerating, progress, generationComplete,
    setFace, setApiKey, setDemo, setStyle, setLight, setFmt, setRes, setRawText,
    startGeneration, regenVar, getVarBlob, testApi, saveConfig,
  };
